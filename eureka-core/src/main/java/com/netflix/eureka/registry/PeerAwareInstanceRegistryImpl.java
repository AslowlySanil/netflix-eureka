...
public class PeerAwareInstanceRegistryImpl extends AbstractInstanceRegistry implements PeerAwareInstanceRegistry {
    ...
    public int syncUp() {
        ...
        for (int i = 0; ((i < serverConfig.getRegistrySyncRetries()) && (count == 0)); i++) {
            if (i > 0) {
                try {
                    //获取其它eureka服务注册表失败后，会等待一段时间再次尝试获取，等待时间默认是30s
            ...
            //从别的服务实例上拉取的服务注册表，判断下是否在本地还没注册。如果还没注册，则在本地进行注册。
            //将这个服务实例的数量，作为eureka server本地初始化的一个服务实例的数量。
            Applications apps = eurekaClient.getApplications();
            for (Application app : apps.getRegisteredApplications()) {
                for (InstanceInfo instance : app.getInstances()) {
                    try {
                        if (isRegisterable(instance)) {
                            //从别的eureka server获取到的服务实例，在自己本地的服务实例注册表中进行注册
                            register(instance, instance.getLeaseInfo().getDurationInSecs(), true);
                            //对拉取到的服务实例进行处理 如果该服务实例还未在本地进行注册，则将服务实例增加到本地注册表
                            //并对计数进行新增操作 返回获取到的服务实例的个数
                            count++;
                        }
                    } catch (Throwable t) {
    ...
    public boolean renew(final String appName, final String id, final boolean isReplication) {
        if (super.renew(appName, id, isReplication)) {
            //然后一样的，将此次的发送心跳动作，同步到集群中的其它的节点上
    ...
    public boolean isLeaseExpirationEnabled() {
        ...
        //numberOfRenewsPerMinThreshold 我期望的一分钟要发送的心跳，比如是
        //getNumOfRenewsInLastMin 上一分钟所有服务实例发送的心跳次数
    ...
    //将该操作同步到其他的eureka的相邻节点上
    //action总共有以下几种 Heartbeat, Register, Cancel, StatusUpdate, DeleteStatusOverride;
    private void replicateToPeers(Action action, String appName, String id,
        ...
        try {
            //isReplication 是否是副本 复制，eureka cient进行操作的时候 isReflication为false，
            //eureka server进行操作的时候，将isReflication设置为true ？？
            if (isReplication) {
            ...
            for (final PeerEurekaNode node : peerEurekaNodes.getPeerEurekaNodes()) {
                ...
                // 如果这个url 代表了这个机器，那么不要进行复制，不要复制自己的数据给自己
