...
public abstract class AbstractInstanceRegistry implements InstanceRegistry {
    ...
    public void register(InstanceInfo registrant, int leaseDuration, boolean isReplication) {
        try {
            ...
            //注册表的数据结果  ConcurrentHashMap<string,Map<string,Lease<InstanceInfo>>>

            read.lock();
            Map<String, Lease<InstanceInfo>> gMap = registry.get(registrant.getAppName());
            REGISTER.increment(isReplication);
            //gmap是空，判断gmap是否为空，如果为空，则手动的创建一个
            if (gMap == null) {
            ...
            //通过服务实例的id 拿到服务实例对应的Lease信息
            Lease<InstanceInfo> existingLease = gMap.get(registrant.getId());
            // Retain the last dirty timestamp without overwriting it, if there is already a lease


            //会先进行判断，判断服务实例数据是否已经注册过了
            //如果Lease不为空，说明之前服务实例已经注册过了
            if (existingLease != null && (existingLease.getHolder() != null)) {
                ...
                //如果Lease为空，说明之前是没有注册过该服务实例的。
                //如果是没有注册过的服务实例 则将服务实例增加到本地的注册表中
                synchronized (lock) {
                    if (this.expectedNumberOfRenewsPerMin > 0) {
                        // Since the client wants to cancel it, reduce the threshold
                        // (1
                        // for 30 seconds, 2 for a minute)
                        //对每分钟期望的心跳次数新增 因为新增了一份服务实例，而且默认是30s心跳一次，
                        //所以每分钟的期望的心跳次数就是 原来的每分钟期望心跳次数+2
                        this.expectedNumberOfRenewsPerMin = this.expectedNumberOfRenewsPerMin + 2;
                        //每分钟的心跳次数的阈值也更新下 *0.85
            ...
            //新建一个lease对象 包含的是服务实例注册信息
            Lease<InstanceInfo> lease = new Lease<InstanceInfo>(registrant, leaseDuration);
            if (existingLease != null) {
                lease.setServiceUpTimestamp(existingLease.getServiceUpTimestamp());
            }
            //将服务实例数据放入到本地服务实例的map中
            gMap.put(registrant.getId(), lease);
            //开启同步锁 并更新最近注册的服务队列 是一种循环队列
            ...
            //更新最近变更的服务队列 即有新的实例增加了
            registrant.setActionType(ActionType.ADDED);
            recentlyChangedQueue.add(new RecentlyChangedItem(lease));
            registrant.setLastUpdatedTimestamp();

            //主动的过期缓存，即eureka client完成实例的注册后，会对eureka server对多级缓存进行处理
    ...
    //取消服务实例注册 取消租约  appName 服务名、id 服务实例id
    ...
    public boolean renew(String appName, String id, boolean isReplication) {
        ...
        //实际的完成服务续约、心跳的逻辑即是更新了服务注册信息的一个最近的更新时间。
        //完成服务续约，发送心跳即 更新任意一个eurekaserver端服务注册表中该服务实例的最近更新时间
        leaseToRenew.renew();
    ...
    public void evict(long additionalLeaseMs) {
        ...
        //跟自我保护机制相关 是否运行主动的删除故障放服务实例
        //自我保护机制 默认是开启的，如果关闭了自我保护机制 就随时可以去完成服务实例的摘除操作
        //如果开启了服务实例的自我保护机制
        ...
        //计算处注册表中所有的服务实例的个数 假设有20个
        int registrySize = (int) getLocalRegistrySize();
        // 默认的配置是0.85   20*0.85=17个
        int registrySizeThreshold = (int) (registrySize * serverConfig.getRenewalPercentThreshold());
        // 20-17=3
        int evictionLimit = registrySize - registrySizeThreshold;

        //比较下实效的那个实例的个数，和设置的一次最多能移除的实例个数。获取两者的最小者
        //比如 实效了比较多的服务实例，不如实效了6个服务实例，而一次最多摘除的服务实例个数是3个。那么就最多能随机摘除3个
        ...
        if (toEvict > 0) {
            ...
            for (int i = 0; i < toEvict; i++) {
                ...
                //再走一般正常的服务实例下线的逻辑处理  从服务实例注册表中删除某个服务实例
    ...
    public Applications getApplicationDeltasFromMultipleRegions(String[] remoteRegions) {
        ...
        try {
            //读写锁的写锁
            write.lock();
            //遍历recentlyChangedQueue队列 拿到的是最近3分钟内的有变化的服务实例信息
            Iterator<RecentlyChangedItem> iter = this.recentlyChangedQueue.iterator();
    ...
    private TimerTask getDeltaRetentionTask() {
        return new TimerTask() {
            ...
            public void run() {
                ...
                while (it.hasNext()) {
                    //循环迭代，若干某个服务实例最近一次的更新时间小于默认的3分钟，那么就表示该服务实例下线了，宕机了。就
                    //从最近变更的服务实例队列中给移除掉。
