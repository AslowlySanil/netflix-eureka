...
public class EurekaBootStrap implements ServletContextListener {
    ...
    public void contextInitialized(ServletContextEvent event) {
        try {
            //创建eureka的配置管理器，使用单例模式创建

            //初始化eureka的运行环境，数据中心+运行环境
    ...
    protected void initEurekaServerContext() throws Exception {
        ...
        //part2 服务器的编码方式，json和xml两种方式的编码方式
        ServerCodecs serverCodecs = new DefaultServerCodecs(eurekaServerConfig);

        /**
         * 第二步 ApplicationInfoManager
         */
        ApplicationInfoManager applicationInfoManager = null;

        //part3 eureka客户端的初始化过程
        if (eurekaClient == null) {
            /**
             * @author cuiyc
             * @date 2021/5/28 22:51
             * @version 1.0
             * @description 初始化eureka实例对象，eurekaInstanceConfig 实例的配置
             */
            ...
            //eureka客户端的配置类  和eurekaServerConfig配置类类似的处理逻辑，通过接口和实现类的方式，
            //完成eurekaClient的各个参数的配置，配置项通过定义常量的方式
            EurekaClientConfig eurekaClientConfig = new DefaultEurekaClientConfig();

            //传入的是applicationInfoManage、eurekaClientConfig 创建一个eureka的客户端
            //加载的是eureka-clent.properties配置文件
        ...
        //part6 初始化eurekaServer的上下文对象 包含了当前服务器需要的所有的信息
        ...
        //part7 eureka服务器的上下文对象放到eurekaServerContextHolder中，可以直接从eurekaServerHolder中获取
        //eurekaContext,在eureka允许的过程中获取 diagrams
        EurekaServerContextHolder.initialize(serverContext);

        //part8 eurekaServer的上下文对象的初始化  initializing
        serverContext.initialize();
        logger.info("Initialized server context");

        // Copy registry from neighboring eureka node

        //part9 向相邻的一个eureka server节点拷贝注册实例对象的数据，如果拷贝失败，再找下一个
        //返回的是获取到服务实例的个数
        int registryCount = registry.syncUp();


        //part10 心跳启动 进行服务实例的注册处理
        registry.openForTraffic(applicationInfoManager, registryCount);

        // Register all monitoring statistics.
        //part11 注册所有的服务监控数据统计
        EurekaMonitors.registerAllStats();
    }

    //获取到eureka server的列表，即eurekaServer1,eurekaServer2,eurekaServer3
