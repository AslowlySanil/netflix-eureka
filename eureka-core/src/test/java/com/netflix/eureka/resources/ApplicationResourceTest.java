...
public class ApplicationResourceTest extends AbstractTester {
    ...
    /*测试一个好的eureka-client注册服务的过程、服务注册表的数据结构树ConcurrentHashMap<string,Map<string,Lease<InstanceInfo>>>
    * InstanceInfo里面包含的数据主要是两个部分 主机名、ip地址、端口号信息 2）租约相关的数据、心跳数据
    * 亮点1 重要的接口，都要对传入的参数进行必要的各种校验和逻辑处理，增加系统的健壮性。
    * 亮点2 ReentrantReadWriteLock 读写锁，多线程环境下控制是否可以并发执行
    * */
    @Test
